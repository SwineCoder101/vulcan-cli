...
Transaction fees and trader-account rent are paid by the trader wallet unless a paymaster is linked with `vulcan wallet set-fee-payer <NAME>` (or overridden per run with the global `--fee-payer <NAME>`). The paymaster must be a stored wallet; it co-signs every transaction as fee payer, the trader wallet still signs, and it gains no authority over funds or positions. When linked, only the paymaster needs SOL. A missing paymaster fails before any network call with `FEE_PAYER_WALLET_NOT_FOUND`; a paymaster that is the active trader wallet is transparently ignored (the trader pays its own fees). An underfunded paymaster fails before signing with `INSUFFICIENT_SOL_FOR_FEE`, showing its balance and the required fee. `vulcan wallet list` / `vulcan_wallet_list` show the linked paymaster. Over MCP, link or unlink it with `vulcan_wallet_set_fee_payer` / `vulcan_wallet_clear_fee_payer`; the link is read at each transaction, so it applies immediately to the running session.
